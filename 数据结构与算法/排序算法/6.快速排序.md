...
public class Sort {
    ...
    private void qSort(int[] datas,int low,int high){
        int pivot;
        if(low<high){
            pivot = Partition(datas,low,high);
            qSort(datas,low,pivot-1);
            qSort(datas,pivot+1,high);
        }
        System.out.println(Arrays.toString(datas));
    }
    private int Partition(int[] datas,int low,int high){
        int pivotkey = datas[low];  //取子表第一个元素作基准
        while(low<high){  //从表的两端交替向中间扫描
            while(low<high && datas[high]>=pivotkey){
                high--;
            }
            swap(datas,low,high);  //将比基准小的交换到低端
            while(low<high && datas[low]<=pivotkey){
                low++;
            }
            swap(datas,low,high);  //将比基准大的交换到高端
        }
        return low;
    }
...
当然可以再写一个函数，用来找到3个数的中位数，也可以直接将原来的代码`int pivotkey = datas[low];`改为
```java
int pivotkey;
...
解决方案：使用替换而不是交换
```java
private int Partition1(int[] datas,int low,int high){
        int pivotkey;
        //省略三数取中法
        pivotkey = datas[low]; 
        ...
        while(low<high){
            ...
            datas[low] = datas[high];  //采用替换而不是交换的方式进行操作
            while(low<high && datas[low]<=pivotkey){
                low++;
            }
            datas[high] = datas[low]; //采用替换而不是交换的方式进行操作
        }
        datas[low] = pivotkey1;  //将基准值替换回datas[low]
        return low;
    }
```
### 3.优化小数组时的排序方案
...
private void qSort(int[] datas,int low,int high){
        ...
        if((high-low)>MAX){  //MAX为设定值
            ...
            insertSort(datas);  //当high-low小于或者等于常数时直接使用插入排序
        }
        System.out.println(Arrays.toString(datas));
    }
```
### 4.聚集相等元素
在一次分割结束后，可以把与pivotkey相等的元素聚在一起，继续下次分割时，不用再对与key相等元素分割,**处理重复元素效率极高** 
...
private void qSort(int[] datas,int low,int high){
        ...
        if((high-low)>MAX){  //MAX为设定值
            while(low<high){
                pivot = Partition1(datas,low,high);
                qSort(datas,low,pivot-1);
                low = pivot+1;  //使用尾递归
            }    
        }else{
...
``` 
当我们将if改成while后，因为第一次递归后，变量low就没有用处了，所以可以量pivot+1赋值给low，再循环来后，来一次partition(datas,low,high),其效果等同于qSort(datas,pivot+1,high)。结果相同，但是因为采用了迭代而不是递归的方法可以缩减堆栈深度从而提高性能。